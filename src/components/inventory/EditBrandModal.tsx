...
export interface EditBrandModalProps {
  isOpen: boolean;
  onClose: () => void;
  brand: { id: number; nombre: string } | null;
...
export function EditBrandModal({
  ...
  // Pre-cargar datos cuando se abre con una marca
  useEffect(() => {
    if (isOpen && brand) {
      reset({ name: brand.nombre });
    }
  }, [isOpen, brand, reset]);
  ...
  const onSubmit = async ({ name }: FormValues) => {
    ...
    try {
      await marcasService.update(brand.id, name);
      onSuccess?.();
      onClose();
    } catch (error) {
      ...
      setError("name", {
        ...
        message:
          error instanceof Error
            ? error.message
            : "No se pudo actualizar la marca. Intentalo de nuevo.",
      });
    }
  };

  return (
    <Modal isOpen={isOpen} onClose={onClose} maxWidth="sm" title="Editar Marca">
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
          ...
          <CapitalizedInput
            ...
            {...register("name", {
              ...
              validate: (value) => {
                ...
                if (
                  nombreLimpio.toLowerCase() === brand?.nombre.toLowerCase()
                ) {
                  return true;
                }
                const esDuplicado = brands.some(
                  (b) =>
                    b.nombre.toLowerCase() === nombreLimpio.toLowerCase(),
                );
        ...
        <div className="mt-2 flex items-center justify-end gap-3 border-t border-slate-100 pt-2 dark:border-slate-800/60">
          ...
          <button
            ...
            className="flex min-w-[140px] cursor-pointer items-center justify-center rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-xs shadow-emerald-900/20 transition-all hover:bg-emerald-500 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 disabled:active:scale-100"
          >
            {isSubmitting ? (
