...
export function Pill({
  ...
  return (
    <div
      className={cn(
        "group relative flex h-8 w-fit shrink-0 select-none items-center rounded-full border text-xs font-semibold transition-all duration-200 whitespace-nowrap",
        active
    ...
    >
      {/*
        La etiqueta es un <button> real, no un div con role="button": teclado,
        foco y rol vienen gratis y no hay que reinventarlos a mano. Las acciones
        van como HERMANAS, nunca dentro: un <button> anidado en otro <button> es
        HTML inválido y rompe la activación por teclado en varios navegadores.
      */}
      <button
        type="button"
        onClick={onSelect}
        ...
        aria-pressed={active}
        className="flex h-full cursor-pointer items-center rounded-full px-3 outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-1 dark:focus-visible:ring-offset-slate-900"
      >
      ...
      {showActions && (
        <div className="flex items-center gap-2 pr-2.5 max-w-0 overflow-hidden opacity-0 transition-all duration-200 ease-out group-hover:max-w-16 group-hover:opacity-100">
